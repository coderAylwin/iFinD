...
STOCK_LISTS = os.getenv('STOCK_LISTS', 'hs300_list.csv')
...
def guess_market(code):
    """根据代码后缀推断市场：cn / us / btc"""
    upper = code.upper()
    if '.SH' in upper or '.SZ' in upper:
        return 'cn'
    if '.O' in upper or '.N' in upper:
        return 'us'
    return 'btc'


def load_stock_lists():
    """从 data/lists/ 目录读取 STOCK_LISTS 配置的清单文件，返回标的代码列表（去重、保序）"""
...
def main():
    ...
    try:
        ...
        print(f"读取到标的: {len(codes)} 只（清单: {STOCK_LISTS}）\n")
